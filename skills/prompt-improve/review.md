# Review — tune the prompt-improve skill
...
1. Read `~/.claude/prompt-improve/runs.jsonl`. Focus on entries since the last
...
4. Apply it, append `## Reviewed up to <date> — <one-line summary>` to the
   Review log below, and commit titled `prompt-improve: tune`.
...
The log lives at `~/.claude/prompt-improve/runs.jsonl` (outside this repo, so
