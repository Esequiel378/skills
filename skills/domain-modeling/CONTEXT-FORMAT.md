...
- **Be opinionated.** When multiple words exist for the same concept, pick the
  best one. List the others under `_Avoid_`.
...
- **Only include terms specific to this project's context.** General
  ...
  this unique to this context, or a general programming concept? Only the
  former belongs.
...
where they live, and how they relate:
...
- If neither exists, create a root `CONTEXT.md` when the first term resolves.
