...
| [`plan-map`](./skills/plan-map/SKILL.md) | Charts work too big for one agent session as a map of decision tickets on GitHub — destination, fog of war, blocking edges, one ticket resolved per session. Adapted from Matt Pocock's [`wayfinder`](https://github.com/mattpocock/skills/blob/main/skills/engineering/wayfinder/SKILL.md). |
